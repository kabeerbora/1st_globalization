...
\label{eq:gravity}
\end{equation}
where $X_{ijt}$ is the bilateral trade flow, $W_{jt}$ is the destination country's annual temperature uncertainty, $\text{SeaDist}_{ij}$ is the shortest sea distance, $\text{Colony}_{ij}$ is the colonial indicator, and $\eta_{ij}$, $\lambda_t$ are origin, destination, and year fixed effects. PPML is used because it is consistent in the presence of heteroskedasticity and zero trade flows, which are prevalent in historical bilateral data.
...
\subsection*{A.3 IV First-Stage Fit and Leave-One-Out Robustness}
...
\subsection*{A.4 Historical Trade Context}
...
\subsection*{A.5 Variable Descriptions and Sources}
